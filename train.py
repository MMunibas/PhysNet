...
parser.add_argument("--num_residual_atomic", type=int,
                    help="number of residual layers for atomic refinements")
parser.add_argument("--num_residual_interaction", type=int,
                    help="number of residual layers for the message phase")
parser.add_argument("--num_residual_output", type=int,
...
parser.add_argument("--learning_rate", default=0.001,
                    type=float, help="learning rate used by the optimizer")
...
parser.add_argument("--nhlambda", type=float,
...
parser.add_argument('--record_run_metadata', type=int,
...
if args.restart is None:
    directory = datetime.utcnow().strftime("%Y%m%d%H%M%S") + "_" + id_generator() + "_F"+str(args.num_features)+"K"+str(args.num_basis)+"b"+str(args.num_blocks)+"a"+str(args.num_residual_atomic)+"i"+str(args.num_residual_interaction) + \
        "o"+str(args.num_residual_output)+"cut"+str(args.cutoff)+"e"+str(args.use_electrostatic)+"d" + \
        str(args.use_dispersion)+"l2"+str(args.l2lambda) + \
        "nh"+str(args.nhlambda)+"keep"+str(args.keep_prob)
else:
...
data_provider = DataProvider(data, args.num_train, args.num_valid,
...
train_queue = DataQueue(data_provider.next_batch,
                        capacity=1000, scope="train_data_queue")
valid_queue = DataQueue(data_provider.next_valid_batch, capacity=args.num_valid //
                        args.valid_batch_size, scope="valid_data_queue")
...
with tf.name_scope("loss"):
    ...
    if data.E is not None:
        ...
        eloss_t, emse_t, emae_t = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
        eloss_v, emse_v, emae_v = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
    # atomic energies
    if data.Ea is not None:
        ...
        ealoss_t, eamse_t, eamae_t = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
        ealoss_v, eamse_v, eamae_v = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
    # forces
    if data.F is not None:
        ...
        floss_t, fmse_t, fmae_t = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
        floss_v, fmse_v, fmae_v = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
    # charge
    if data.Q is not None:
        ...
        qloss_t, qmse_t, qmae_t = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
        qloss_v, qmse_v, qmae_v = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
    # atomic charges
    if data.Qa is not None:
        ...
        qaloss_t, qamse_t, qamae_t = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
        qaloss_v, qamse_v, qamae_v = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
    # dipole
    if data.D is not None:
        ...
        dloss_t, dmse_t, dmae_t = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
        dloss_v, dmse_v, dmae_v = tf.constant(
            0.0), tf.constant(0.0), tf.constant(0.0)
    ...
    l2loss = tf.reduce_mean(tf.get_collection(
        tf.GraphKeys.REGULARIZATION_LOSSES))
    loss_t = eloss_train + args.force_weight*floss_train + args.charge_weight*qloss_train + \
        args.dipole_weight*dloss_train + args.nhlambda*nhloss_t + args.l2lambda*l2loss
    loss_v = eloss_valid + args.force_weight*floss_valid + args.charge_weight*qloss_valid + \
        args.dipole_weight*dloss_valid + args.nhlambda*nhloss_v + args.l2lambda*l2loss

# create trainer
trainer = Trainer(args.learning_rate, args.decay_steps,
                  args.decay_rate, scope="trainer")
...
summary_writer = tf.summary.FileWriter(
    logdir=log_dir, graph=tf.get_default_graph())
...
with tf.Session() as sess:
    ...
    if checkpoint is not None:
        # reads step from checkpoint filename
    ...
    while not coord.should_stop():
        ...
        _, tmploss, emse, emae, fmse, fmae, qmse, qmae, dmse, dmae = sess.run(
            [train_op, loss_t, emse_t, emae_t, fmse_t, fmae_t, qmse_t, qmae_t, dmse_t, dmae_t], options=run_options, feed_dict={nn.keep_prob: args.keep_prob}, run_metadata=run_metadata)

        # update averages
        num_t, tmploss_avg_t, emse_avg_t, emae_avg_t, fmse_avg_t, fmae_avg_t, qmse_avg_t, qmae_avg_t, dmse_avg_t, dmae_avg_t = update_averages(
            num_t, tmploss_avg_t, tmploss, emse_avg_t, emse, emae_avg_t, emae, fmse_avg_t, fmse, fmae_avg_t, fmae, qmse_avg_t, qmse, qmae_avg_t, qmae, dmse_avg_t, dmse, dmae_avg_t, dmae)
        ...
        if (step % args.validation_interval == 0):
            ...
            for i in range(args.num_valid//args.valid_batch_size):
                tmploss, emse, emae, fmse, fmae, qmse, qmae, dmse, dmae = sess.run(
                    [loss_v, emse_v, emae_v, fmse_v, fmae_v, qmse_v, qmae_v, dmse_v, dmae_v])
                num_v, tmploss_avg_v, emse_avg_v, emae_avg_v, fmse_avg_v, fmae_avg_v, qmse_avg_v, qmae_avg_v, dmse_avg_v, dmae_avg_v = update_averages(
                    num_v, tmploss_avg_v, tmploss, emse_avg_v, emse, emae_avg_v, emae, fmse_avg_v, fmse, fmae_avg_v, fmae, qmse_avg_v, qmse, qmae_avg_v, qmae, dmse_avg_v, dmse, dmae_avg_v, dmae)
        ...
        if (step % args.summary_interval == 0) and (step > 0):
            ...
            if data.E is not None:
                print(str(step)+'/'+str(args.max_steps), "loss:",
                      results["loss_train"], "best:", best_loss, "emae:", results["energy_mae_train"], "best:", best_emae)
