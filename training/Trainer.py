...
class Trainer:
    '''Trainer for high level interface to train model'''
    ...
    def __init__(self, learning_rate=1e-3, decay_steps=100000, decay_rate=0.96, scope=None):
        ...
        with tf.variable_scope(self.scope):
            ...
            self._learning_rate = tf.train.exponential_decay(
                learning_rate, self._global_step, decay_steps, decay_rate)
    ...
    def build_train_op(self, loss, moving_avg_decay=0.999, max_norm=10.0, dependencies=[]):
        ...
        summary_op = tf.summary.scalar(
            "global_gradient_norm", tf.global_norm(gradients))
        gradients, _ = tf.clip_by_global_norm(gradients, max_norm)
        apply_gradient_op = self._optimizer.apply_gradients(
        ...
        self._ema = tf.train.ExponentialMovingAverage(
            moving_avg_decay, self._global_step)
