...
class NeuralNetwork:
    ...
    def __init__(self,
                 ...
                 # number of residual layers for atomic refinements of feature vector
                 ...
                 # initial value for output energy shift (makes convergence faster)
                 Eshift=0.0,
                 # initial value for output energy scale (makes convergence faster)
                 Escale=1.0,
                 Qshift=0.0,  # initial value for output charge shift
                 Qscale=1.0,  # initial value for output charge scale
                 # half (else double counting) of the Coulomb constant (default is in units e=1, eV=1, A=1)
        ...
        with tf.variable_scope(self.scope):
            ...
            self._keep_prob = tf.placeholder_with_default(
                1.0, shape=[], name="keep_prob")

            # atom embeddings (we go up to Pu(94), 95 because indices start with 0)
            self._embeddings = tf.Variable(tf.random_uniform([95, self.F], minval=-np.sqrt(
                3), maxval=np.sqrt(3), seed=seed, dtype=dtype), name="embeddings", dtype=dtype)
            ...
            if s6 is None:
                self._s6 = tf.nn.softplus(tf.Variable(softplus_inverse(
                    d3_s6), name="s6", dtype=dtype, trainable=True))
            else:
                self._s6 = tf.Variable(
                    s6, name="s6", dtype=dtype, trainable=False)
            tf.summary.scalar("d3-s6", self.s6)
            if s8 is None:
                self._s8 = tf.nn.softplus(tf.Variable(softplus_inverse(
                    d3_s8), name="s8", dtype=dtype, trainable=True))
            else:
                self._s8 = tf.Variable(
                    s8, name="s8", dtype=dtype, trainable=False)
            tf.summary.scalar("d3-s8", self.s8)
            if a1 is None:
                self._a1 = tf.nn.softplus(tf.Variable(softplus_inverse(
                    d3_a1), name="a1", dtype=dtype, trainable=True))
            else:
                self._a1 = tf.Variable(
                    a1, name="a1", dtype=dtype, trainable=False)
            tf.summary.scalar("d3-a1", self.a1)
            if a2 is None:
                self._a2 = tf.nn.softplus(tf.Variable(softplus_inverse(
                    d3_a2), name="a2", dtype=dtype, trainable=True))
            else:
                self._a2 = tf.Variable(
                    a2, name="a2", dtype=dtype, trainable=False)
            tf.summary.scalar("d3-a2", self.a2)

            # initialize output scale/shift variables
            self._Eshift = tf.Variable(tf.constant(
                Eshift, shape=[95], dtype=dtype), name="Eshift", dtype=dtype)
            self._Escale = tf.Variable(tf.constant(
                Escale, shape=[95], dtype=dtype), name="Escale", dtype=dtype)
            self._Qshift = tf.Variable(tf.constant(
                Qshift, shape=[95], dtype=dtype), name="Qshift", dtype=dtype)
            self._Qscale = tf.Variable(tf.constant(
                Qscale, shape=[95], dtype=dtype), name="Qscale", dtype=dtype)
            ...
            self._saver = tf.train.Saver(
                self.variables, save_relative_paths=True, max_to_keep=50)
    ...
    def atomic_properties(self, Z, R, idx_i, idx_j, offsets=None, sr_idx_i=None, sr_idx_j=None, sr_offsets=None):
        ...
        with tf.name_scope("atomic_properties"):
            ...
            Ea = tf.gather(self.Escale, Z) * Ea + \
                tf.gather(self.Eshift, Z) + 0*tf.reduce_sum(R, -1)
        ...
        with tf.name_scope("energy_from_atomic_properties"):
            ...
            if self.use_dispersion:
                if self.lr_cut is not None:
                    Ea += d3_autoev*edisp(Z, Dij/d3_autoang, idx_i, idx_j, s6=self.s6,
                                          s8=self.s8, a1=self.a1, a2=self.a2, cutoff=self.lr_cut/d3_autoang)
                else:
                    Ea += d3_autoev * \
                        edisp(Z, Dij/d3_autoang, idx_i, idx_j, s6=self.s6,
                              s8=self.s8, a1=self.a1, a2=self.a2)
    ...
    def energy_and_forces_from_scaled_atomic_properties(self, Ea, Qa, Dij, Z, R, idx_i, idx_j, batch_seg=None):
        ...
        with tf.name_scope("energy_and_forces_from_atomic_properties"):
            ...
            forces = - \
                tf.convert_to_tensor(tf.gradients(tf.reduce_sum(energy), R)[0])
    ...
    def energy_and_forces(self, Z, R, idx_i, idx_j, Q_tot=None, batch_seg=None, offsets=None, sr_idx_i=None, sr_idx_j=None, sr_offsets=None):
        ...
        with tf.name_scope("energy_and_forces"):
            Ea, Qa, Dij, _ = self.atomic_properties(
                Z, R, idx_i, idx_j, offsets, sr_idx_i, sr_idx_j, sr_offsets)
            energy, forces = self.energy_and_forces_from_atomic_properties(
                Ea, Qa, Dij, Z, R, idx_i, idx_j, Q_tot, batch_seg)
        return energy, forces
    ...
    def scaled_charges(self, Z, Qa, Q_tot=None, batch_seg=None):
        ...
        with tf.name_scope("scaled_charges"):
            ...
            Na_per_batch = tf.segment_sum(tf.ones_like(
                batch_seg, dtype=self.dtype), batch_seg)
    ...
    def variables(self):
        ...
        varlist = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope=scope_filter)
