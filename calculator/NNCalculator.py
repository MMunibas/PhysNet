...
class NNCalculator:
    ...
    def __init__(self,
                 # ckpt file from which to restore the model (can also be a list for ensembles)
                 checkpoint,
        ...
        self._offsets = tf.placeholder(
            dtype,    shape=[None, 3], name="offsets")
        self._sr_idx_i = tf.placeholder(
            tf.int32, shape=[None, ], name="sr_idx_i")
        self._sr_idx_j = tf.placeholder(
            tf.int32, shape=[None, ], name="sr_idx_j")
        self._sr_offsets = tf.placeholder(
            dtype,    shape=[None, 3], name="sr_offsets")

        # calculate atomic charges, energy and force evaluation nodes
        if self.use_neighborlist:
            Ea, Qa, Dij, nhloss = self.nn.atomic_properties(
                self.Z, self.R, self.idx_i, self.idx_j, self.offsets, self.sr_idx_i, self.sr_idx_j, self.sr_offsets)
        else:
            Ea, Qa, Dij, nhloss = self.nn.atomic_properties(
                self.Z, self.R, self.idx_i, self.idx_j, self.offsets)
        self._charges = self.nn.scaled_charges(self.Z, Qa, self.Q_tot)
        self._energy, self._forces = self.nn.energy_and_forces_from_scaled_atomic_properties(
            Ea, self.charges, Dij, self.Z, self.R, self.idx_i, self.idx_j)
    ...
    def _calculate_all_properties(self, atoms):
        ...
        if (type(self.checkpoint) is not list):  # only one NN
            self._last_energy, self._last_forces, self._last_charges = self.sess.run(
                [self.energy, self.forces, self.charges], feed_dict=feed_dict)
            self._energy_stdev = 0
        else:  # ensemble is used
            for i in range(len(self.checkpoint)):
                self.nn.restore(self.sess, self.checkpoint[i])
                energy, forces, charges = self.sess.run(
                    [self.energy, self.forces, self.charges], feed_dict=feed_dict)
                if i == 0:
                    ...
                    for a in range(np.shape(charges)[0]):  # loop over atoms
                        self._last_charges[a] += (charges[a] -
                                                  self.last_charges[a])/n
                        for b in range(3):
                            self._last_forces[a,
                                              b] += (forces[a, b]-self.last_forces[a, b])/n
...
if (__name__ == "__main__"):
    if (len(sys.argv) != 3):
        print("Provide path to checkpoint and atom")
    checkpoint_path = sys.argv[1]
    atom_path = sys.argv[2]

    atom = pickle.load(open(atom_path, 'rb'))
    calc = NNCalculator(checkpoint=checkpoint_path, atoms=atom)
    print(calc.get_potential_energy(atom)[0])
